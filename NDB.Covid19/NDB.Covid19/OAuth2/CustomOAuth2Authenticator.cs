...
namespace NDB.Covid19.OAuth2
{
    public class CustomOAuth2Authenticator : OAuth2Authenticator
    {
        ...
        public CustomOAuth2Authenticator(string clientId,
            ...
            bool isUsingNativeUI = false) : base(clientId, scope, authorizeUrl, redirectUrl, getUsernameAsync,
            isUsingNativeUI)
        {
        }
        ...
        public CustomOAuth2Authenticator(
            ...
            Uri accessTokenUrl,
            GetUsernameAsyncFunc getUsernameAsync = null,
            bool isUsingNativeUI = false) : base(clientId, clientSecret, scope, authorizeUrl, redirectUrl,
            accessTokenUrl, getUsernameAsync, isUsingNativeUI)
        {
        ...
        protected override async void OnRedirectPageLoaded(Uri url, IDictionary<string, string> query,
            IDictionary<string, string> fragment)
        {
            ...
            catch (AuthException e)
            {
                LogUtils.LogException(LogSeverity.ERROR, e,
                    $"{nameof(CustomOAuth2Authenticator)}-{nameof(OnRedirectPageLoaded)} threw an exception");
        ...
        public async Task<IDictionary<string, string>> CustomRequestAccessTokenAsync(
            IDictionary<string, string> queryValues)
        {
            ...
            try
            {
                ...
                {
                    LogUtils.LogMessage(LogSeverity.WARNING, "CustomOAuth2Authenticator failed to refresh token.",
                        "Error from service: " + text);
                    Debug.Print("Error from service: " + text);
                }
            }
            catch
            {
            ...
            if (data.ContainsKey(AccessTokenName))
                //---------------------------------------------------------------------------------------
            {
            }
            else
            {
                ...
                throw new AuthException("Expected " + AccessTokenName +
                                        " in access token response, but did not receive one.");
                //---------------------------------------------------------------------------------------
            }
        ...
        private string CreateChallenge(string code)
        {
            ...
            var challengeByteArray =
                sha256.HashData(
                    WinRTCrypto.CryptographicBuffer.CreateFromByteArray(Encoding.UTF8.GetBytes(codeVerifier)));
