    ...
    public class TimeoutErrorHandler : BaseErrorHandler, IErrorHandler
    {
        ...
        public bool IsResponsible(ApiResponse apiResponse)
        {
            return apiResponse.Exception != null
                   &&
                   apiResponse.Exception is
                       TaskCanceledException; //As long as we never cancel tasks manually, then we can expect that this always means time out.
        }

        public void HandleError(ApiResponse apiResponse)
        {
            string message =
                $"{apiResponse.ErrorLogMessage}. Timed out after {Conf.DEFAULT_TIMEOUT_SERVICECALLS_SECONDS} seconds because of bad connection.";
            LogUtils.LogApiError(LogSeverity.WARNING, apiResponse, IsSilent, "", message);
            if (!IsSilent)
            {
                ShowErrorToUser();
            }
