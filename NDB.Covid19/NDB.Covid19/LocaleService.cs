    ...
    public static class LocalesService
    {
        public static void Initialize()
        {
            if (I18N.Current?.Locale == null)
            {
                I18N.Current
                    .SetNotFoundSymbol(
                        "$") // Optional: when a key is not found, it will appear as $key$ (defaults to "$")
                    .SetFallbackLocale(Conf
                        .DEFAULT_LANGUAGE) // Optional but recommended: locale to load in case the system locale is not supported
        ...
        public static string GetLanguage()
        {
            ...
            bool currentCultureIsSupported =
                Conf.SUPPORTED_LANGUAGES.Contains(CultureInfo.CurrentCulture.TwoLetterISOLanguageName);
