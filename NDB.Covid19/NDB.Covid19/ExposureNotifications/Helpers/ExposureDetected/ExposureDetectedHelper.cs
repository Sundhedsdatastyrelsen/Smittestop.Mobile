...
namespace NDB.Covid19.ExposureNotifications.Helpers.ExposureDetected
{
    ...
    {
        private static readonly string _logPrefix = $"{nameof(ExposureDetectedHelper)}";
        ...
        public static bool HasNotShownExposureNotificationForDate(DateTime dateOfExposure,
            List<DateTime> previouslySavedDatesOfExposures)
        {
        ...
        public static void SaveLastSummary(ExposureDetectionSummary summary)
        {
            ...
            {
                LogUtils.LogException(LogSeverity.ERROR, e, $"{_logPrefix}.{nameof(SaveLastSummary)}");
            }
        }
        ...
        public static List<DateTime> DeleteDatesOfExposureOlderThan14DaysAndReturnNewList()
        {
            try
            {
                ...
                {
                    string datesOfExposuresSavedEarlier =
                        _secureStorageService.GetValue(SecureStorageKeys.DAILY_SUMMARIES_OVER_THRESHOLD_TIMESTAMP_KEY);
                    List<DateTime> datesOfPreviousExposures =
                        JsonConvert.DeserializeObject<List<DateTime>>(datesOfExposuresSavedEarlier);
                    ...
                    _secureStorageService.SaveValue(SecureStorageKeys.DAILY_SUMMARIES_OVER_THRESHOLD_TIMESTAMP_KEY,
                        datesAsString);
                ...
                LogUtils.LogMessage(LogSeverity.INFO,
                    $"{_logPrefix}.DeleteDatesOfExposureOlderThan14Days: No exposures discovered yet using EN API v2");
                return new List<DateTime>();
            }
            catch (Exception e)
            {
                LogUtils.LogException(LogSeverity.ERROR, e, $"{_logPrefix}.DeleteDatesOfExposureOlderThan14Days: " +
        ...
        public static async Task UpdateDatesOfExposures(List<DateTime> datesOfExposuresOverThreshold)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    {
                        string datesOfExposuresSavedEarlier =
                            _secureStorageService.GetValue(SecureStorageKeys
                                .DAILY_SUMMARIES_OVER_THRESHOLD_TIMESTAMP_KEY);
                        List<DateTime> datesOfPreviousExposures =
                            JsonConvert.DeserializeObject<List<DateTime>>(datesOfExposuresSavedEarlier);
                        List<DateTime> datesOfPreviousExposuresAndNewlyDiscovered =
                            datesOfPreviousExposures.Concat(datesOfExposuresOverThreshold).ToList();
                        string datesAsString = JsonConvert.SerializeObject(datesOfPreviousExposuresAndNewlyDiscovered);
                        LogUtils.LogMessage(LogSeverity.INFO,
                            $"{_logPrefix}.UpdateDatesOfExposure: Dates: {datesAsString}");
                        _secureStorageService.SaveValue(SecureStorageKeys.DAILY_SUMMARIES_OVER_THRESHOLD_TIMESTAMP_KEY,
                            datesAsString);
                    }
                    else
                    {
                        LogUtils.LogMessage(LogSeverity.INFO,
                            $"{_logPrefix}.UpdateDatesOfExposure: No exposures discovered yet using EN API v2");
                ...
                {
                    LogUtils.LogException(LogSeverity.ERROR, e, $"{_logPrefix}.UpdateDatesOfExposure: " +
                                                                "Unexpected error has occured when saving Exposure Dates to Secure Storage. " +
                                                                "Resetting the saved value to avoid error in the future.");
                    _secureStorageService.Delete(SecureStorageKeys.DAILY_SUMMARIES_OVER_THRESHOLD_TIMESTAMP_KEY);
                }
