...
namespace NDB.Covid19.ExposureNotifications.Helpers
{
    public abstract class BatchFileHelper
    {
        public static IEnumerable<string> SaveZipStreamToBinAndSig(Stream zipStream)
        {
            ...
            string exportBinTmp = Path.Combine(ServiceLocator.Current.GetInstance<IFileSystem>().CacheDirectory,
                Guid.NewGuid() + ".bin");
            string exportSigTmp = Path.Combine(ServiceLocator.Current.GetInstance<IFileSystem>().CacheDirectory,
                Guid.NewGuid() + ".sig");
