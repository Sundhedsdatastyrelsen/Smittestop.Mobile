...
namespace NDB.Covid19.ExposureNotifications.Helpers.FetchExposureKeys
{
    public class ZipDownloader
    {
        ...
        private readonly string _logPrefix = $"{nameof(ZipDownloader)}";
        ...
        /// </summary>
        /// <returns>Paths to the temporary location where the zips are stored</returns>
        public async Task<IEnumerable<string>> PullNewKeys(ExposureNotificationWebService service,
            ...
            while (!lastPull)
            {
                ...
                if (response.StatusCode == 204)
                {
                    ...
                    else
                    {
                        ...
                        string warning =
                            $"API {response.Endpoint} returned 204 No Content - No new keys since last pull";
                        LogUtils.LogMessage(LogSeverity.WARNING, $"{_logPrefix}.{nameof(DownloadZips)}: {warning}");
                        lastPull = true;
                    }
                }
                else
                {
                    ...
                    catch (Exception e)
                    {
                        ...
                        HandleErrorWhenPulling(e,
                            $"Failed to parse {MoreBatchesExistHeader} or {LastBatchReturnedHeader} header.",
                            requestUrl);
                        break; //Abort pulling
                    }
                }
                ...
                if (response.StatusCode == 200 && headersAreValid)
                {
                    try
                    {
                        ...
                        string tmpFile = Path.Combine(ServiceLocator.Current.GetInstance<IFileSystem>().CacheDirectory,
                            Guid.NewGuid() + ".zip");
