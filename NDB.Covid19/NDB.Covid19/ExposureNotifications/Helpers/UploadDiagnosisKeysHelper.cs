...
namespace NDB.Covid19.ExposureNotifications.Helpers
{
    public abstract class UploadDiagnosisKeysHelper
    {
	    ...
	    /// <returns></returns>
	    public static List<ExposureKeyModel> CreateAValidListOfTemporaryExposureKeys(
            IEnumerable<ExposureKeyModel> temporaryExposureKeys)
        {
            ...
            if (validListOfTeks.Count() != temporaryExposureKeys.Count())
            {
                LogUtils.LogMessage(LogSeverity.WARNING,
                    "Number of keys filtered out by Smittestopp: " +
                    (temporaryExposureKeys.Count() - validListOfTeks.Count()));
            }
	    ...
	    ///     It operates on intervals that are set in configuration file, these intervals are defined strictly, without any
	    ///     overlaps.
	    ///     The method does convertion from UTC time representation of symptomOnset date to DateTimeOffset, then gets
	    ///     a positive or negative integer representing difference between key's DateTimeOffset and the symptoms
	    ///     DateTimeOffset.
	    ///     Based on configuration, TransmissionRiskLevel for the key is then set to the natural index in the table
	    ///     TransmissionRiskLevel table.
	    ...
	    ///     NOTE: The key risk will always be assigned to 1 (lowest risk value) at the end, if none other higher value is
	    ///     assigned.
	    ...
	    /// <returns></returns>
	    public static List<ExposureKeyModel> SetTransmissionRiskAndDSOS(List<ExposureKeyModel> keys,
            DateTime symptomsDate)
        {
            ...
            foreach (ExposureKeyModel key in keys)
            {
                ...
                {
                    if (daysBetweenSymptomOnsetAndKey >=
                        Conf.DAYS_SINCE_ONSET_FOR_TRANSMISSION_RISK_CALCULATION[i].Item1
                        && daysBetweenSymptomOnsetAndKey <=
                        Conf.DAYS_SINCE_ONSET_FOR_TRANSMISSION_RISK_CALCULATION[i].Item2)
                    {
                        key.TransmissionRiskLevel =
                            (RiskLevel) i + 1; // Setting to natural index number since 0 represents Invalid
                        break;
                    }
                }
            }
