...
namespace NDB.Covid19.ViewModels
{
    public class ENDeveloperToolsViewModel
    {
        ...
        private readonly string _logPrefix = $"{nameof(ENDeveloperToolsViewModel)}: ";
        ...
        internal static void UpdatePushKeysInfo(ApiResponse response,
            SelfDiagnosisSubmissionDTO selfDiagnosisSubmissionDTO, JsonSerializerSettings settings)
        {
            PushKeysInfo =
        ...
        private static void ParseKeys(SelfDiagnosisSubmissionDTO selfDiagnosisSubmissionDTO,
            JsonSerializerSettings settings, ENOperation varAssignCheck)
        {
        ...
        public async Task<bool> PullKeysFromServer()
        {
            ...
            try
            {
                await EN.ExposureNotification.UpdateKeysFromServer();
            }
            catch (Exception e)
            {
        ...
        public string GetExposureInfosFromLastPull()
        {
            ...
            else
            {
                try
                {
                    IEnumerable<EN.ExposureInfo> exposureInfos =
                        ExposureInfoJsonHelper.ExposureInfosFromJsonCompatibleString(exposureInfosString);
                    foreach (EN.ExposureInfo exposureInfo in exposureInfos)
                    {
            ...
            string finalResult =
                $"These are the ExposureInfos we got the last time \"Pull keys and get exposure info\" was clicked:\n{result}";
        ...
        public async Task<string> FetchExposureConfigurationAsync()
        {
            try
            {
                EN.DailySummaryConfiguration dsc =
                    await new ExposureNotificationWebService().GetDailySummaryConfiguration();
            ...
            catch (InvalidOperationException)
            {
                ...
                string result =
                    $" AttenuationWeight: {c.AttenuationWeight}, Values: {ENConfArrayString(c.AttenuationScores)} \n" +
        ...
        public string DecrementExposureDate()
        {
            _messageDateTime = _messageDateTime.AddDays(-1);
            return
                $"Decremented date for Send Message function: \n{_messageDateTime.ToGreGorianUtcString("yyyy-MM-dd HH:mm:ss")}";
        }
        ...
        {
            string result;
            if (ServiceLocator.Current.GetInstance<SecureStorageService>()
                .KeyExists(SecureStorageKeys.LAST_SUMMARY_KEY))
            {
                result = "Last exposure summary: " + ServiceLocator.Current.GetInstance<SecureStorageService>()
                    .GetValue(SecureStorageKeys.LAST_SUMMARY_KEY);
            }
            else
            {
