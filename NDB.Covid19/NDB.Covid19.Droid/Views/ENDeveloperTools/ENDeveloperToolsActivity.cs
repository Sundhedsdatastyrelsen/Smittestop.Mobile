...
namespace NDB.Covid19.Droid.Views.ENDeveloperTools
{
    ...
    public class ENDeveloperToolsActivity : AppCompatActivity
    {
        ...
        private SingleClick OnPullWithDelay()
        {
            return new SingleClick((o, args) =>
                _viewModel.PullWithDelay(_viewModel.PullKeysFromServer));
        }

        private SingleClick OnFakeGateway()
        {
            return new SingleClick((o, args) =>
            {
                RunOnUiThread(() =>
                {
                    ...
                    input.LayoutParameters = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MatchParent,
                    ...
                    new AlertDialog.Builder(this)
                        ...
                        .SetNegativeButton("Cancel", (sender, eventArgs) =>
                        {
                            /* ignore */
                        })
                        .Show();
        ...
        private SingleClick OnPrintActualPreferences()
        {
            return new SingleClick(async (o, args) => { UpdateText(await _viewModel.GetFormattedPreferences()); });
        }
        ...
        private void FetchExposureConfiguration()
        {
            Task.Run(async () =>
            {
                ...
                RunOnUiThread(() => { UpdateText("Copied to clipboard:\n" + res); });
            });
        }
        ...
        private void ResetLocalData()
        {
            ...
            DialogUtils.DisplayDialogAsync(this, "Local data partially deleted",
                "Delete successful. You still need to reinstall the app to delete Exposure Notification history and to avoid bugs where the app doesn't know we have exceeded the 'provide keys' quota for the past 24 hours",
                "OK");
        }
