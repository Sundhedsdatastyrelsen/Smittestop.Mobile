...
namespace NDB.Covid19.Droid.Views.AuthenticationFlow.ErrorActivities
{
    ...
    internal class GeneralErrorActivity : AppCompatActivity
    {
        ...
        private void Init()
        {
            ...
            errorDescription.MovementMethod = LinkMovementMethod.Instance;

            ViewGroup close = FindViewById<ViewGroup>(Resource.Id.close_cross_btn);
            close.Click += new SingleClick((o, ev) => { NavigationHelper.GoToResultPageAndClearTop(this); }).Run;
            ...
            button.Click += new SingleClick((o, ev) => { NavigationHelper.GoToResultPageAndClearTop(this); }).Run;

            Title = textFieldsBundle.GetString("title");
        }
