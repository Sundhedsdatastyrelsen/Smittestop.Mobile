...
namespace NDB.Covid19.Droid.Utils
{
    ...
    {
        private static readonly string _logPrefix = $"Android {nameof(DiagnosisKeysDataMappingUtils)}: ";
        ...
        public static async Task SetDiagnosisKeysDataMappingAsync()
        {
            ...
            catch (ApiException e)
            {
                ...
                if (e.StatusCode == CommonStatusCodes.ApiNotConnected)
                {
                    LogUtils.LogException(LogSeverity.WARNING, e,
                        _logPrefix + "SetDiagnosisKeysDataMappingAsync is not available in EN API v1, aborting.");
                    return;
                }
            }
            ...
            if ((SystemTime.Now() - LocalPreferencesHelper.GetLastDiagnosisKeysDataMappingDateTime()).TotalDays <= 7)
            {
                LogUtils.LogMessage(LogSeverity.WARNING,
                    _logPrefix + "SetDiagnosisKeysDataMappingAsync called too early, aborting.");
                return;
            }

            EN.DailySummaryConfiguration config =
                await new ExposureNotificationWebService().GetDailySummaryConfiguration();
            DiagnosisKeysDataMapping.DiagnosisKeysDataMappingBuilder builder =
                new DiagnosisKeysDataMapping.DiagnosisKeysDataMappingBuilder();
            JavaDictionary<Integer, Integer> newMap = new JavaDictionary<Integer, Integer>();
            foreach (var pair in config.DaysSinceOnsetInfectiousness)
            {
                newMap[new Integer(pair.Key)] = new Integer((int) pair.Value);
            }
            ...
            catch (Exception e)
            {
                ...
                LogUtils.LogException(LogSeverity.ERROR, e,
                    _logPrefix + "Failed to call SetDiagnosisKeysDataMappingAsync.");
                return;
            }

            LocalPreferencesHelper.UpdateLastDiagnosisKeysDataMappingDateTime();
            LogUtils.LogMessage(LogSeverity.INFO,
                _logPrefix + "Successfully updated the mapping with SetDiagnosisKeysDataMappingAsync");
        }
    }
}