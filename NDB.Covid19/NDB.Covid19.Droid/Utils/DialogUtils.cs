...
namespace NDB.Covid19.Droid.Utils
{
    ...
    {
        private static readonly string _logPrefix = $"Android {nameof(DialogUtils)}: ";

        [Obsolete("Use DisplayDialogAsync instead")]
        public static void DisplayDialog(Activity current, string title, string message, string okBtnText,
            Action action = null)
        {
            ...
            int? noBtnTextResourceId = null)
        {
                ...
                if (string.IsNullOrEmpty(okBtnText) == false)
                {
                    dialog
                        .SetPositiveButton(okBtnText,
                            (sender, e) => { SetResultWithCatch(tcs, true, additionalInfo); });
                }

                if (okBtnTextResourceId != null)
                {
                    dialog
                        .SetPositiveButton(okBtnTextResourceId.Value,
                            (sender, e) => { SetResultWithCatch(tcs, true, additionalInfo); });
                }
            }
            ...
            if (string.IsNullOrEmpty(noBtnText) == false || noBtnTextResourceId != null)
            {
                ...
                {
                    dialog
                        .SetNegativeButton(noBtnText,
                            (sender, e) => { SetResultWithCatch(tcs, false, additionalInfo); });
                }

                if (noBtnTextResourceId != null)
                {
                    dialog
                        .SetNegativeButton(noBtnTextResourceId.Value,
                            (sender, e) => { SetResultWithCatch(tcs, false, additionalInfo); });
                }
            }
        ...
        private static string AdditionalDialogInfoForLogging(Activity current, string title, string message,
            string okBtnText, string noBtnText)
        {
            ...
            catch (Exception e)
            {
                if (e is InvalidOperationException)
                {
                    LogUtils.LogException(LogSeverity.WARNING, e,
                        _logPrefix +
                        "InvalidOperationException, this indicates the task was already in a final state when SetResult was called, (User most likely pressed both button at the same time)",
                        additionalInfo);
                }
                else if (e is ObjectDisposedException)
                {
                    LogUtils.LogException(LogSeverity.ERROR, e,
                        _logPrefix +
                        "ObjectDisposedException, this indicates the task was disposed when SetResult was called.",
                        additionalInfo);
                }
                else
                {
                    LogUtils.LogException(LogSeverity.ERROR, e, _logPrefix + "Some Exception from SetResult operation.",
                        additionalInfo);
                }
            }
        }

        public static async Task<bool> DisplayDialogAsync(Activity current, DialogViewModel viewModel,
            Action actionOk = null, Action actionNotOk = null)
        {
