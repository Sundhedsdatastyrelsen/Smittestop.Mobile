...
namespace NDB.Covid19.Droid.Utils
{
    public abstract class PlayServicesVersionUtils
    {
        public static bool PlayServicesVersionNumberIsLargeEnough(PackageManager packageManager)
        {
            ...
            long playServicesVersionNumber = PackageInfoCompat.GetLongVersionCode(
                CrossCurrentActivity.Current.AppContext.PackageManager.GetPackageInfo(
                    GoogleApiAvailability.GooglePlayServicesPackage, 0));
            bool isLargeEnough = playServicesVersionNumber >= minimumPlayServicesVersionNumber;
            if (!isLargeEnough)
            {
                LogUtils.LogMessage(LogSeverity.INFO,
                    "PlayServicesVersionUtils: User is prevented from using the app because of too low GPS version");
            }
