...
namespace NDB.Covid19.iOS.Views.ConsentView
{
    public partial class ConsentViewController : BaseViewController
    {
        ...
        private void SetTexts()
        {
            ...
            WarningLbl.Font = Font(FontType.FontRegular, 12, 16);
            ...
            InvokeOnMainThread(() =>
            {
                ...
                catch (Exception e)
                {
                    LogUtils.LogMessage(LogSeverity.WARNING,
                        $"Onboarding failed: Failed to retrieve Device ID from server {e.Message}");
                }
            });
        }

        public void SetWarningViewVisibility()
        {
            WarningView.Alpha = _consentViewModel.ConsentIsGiven ? 0 : 100;
        }
        ...
        private void InitPrivacyPolicyButton()
        {
            ...
            PrivacyPolicy.OnFocus += () => { BottomControlsView.UserInteractionEnabled = false; };
            PrivacyPolicy.OnFocusLost += () => { BottomControlsView.UserInteractionEnabled = true; };
            InitButtonStyling(PrivacyPolicy, ConsentViewModel.CONSENT_SEVEN_BUTTON_TEXT);
        }

        partial void PrivacyPolicy_TouchUpInside(FocusableButton sender)
        {
