    ...
    public class MigrationHelperTest
    {
        ...
        public void MigrateSucessfullyFromDanishToDanish_LAST_DOWNLOAD_ZIPS_CALL_UTC_DATETIME_PREF()
        {
            ...
            var actual = preferences
                .Get(PreferencesKeys.LAST_DOWNLOAD_ZIPS_CALL_UTC_DATETIME_PREF, SystemTime.Now().AddDays(-300))
                .TrimMilliseconds();
        ...
        {
            if (Environment.OSVersion.ToString() !=
                "Unix 19.5.0.0") //This test fails on Mac, but still runs find in CI tests
            {
                //Arrange
        ...
        public void MigrateSuccessfullyFromToDanishToDanish_CURRENT_DAY_TO_DOWNLOAD_KEYS_FOR_UTC_PREF()
        {
            ...
            var actual = preferences.Get(PreferencesKeys.CURRENT_DAY_TO_DOWNLOAD_KEYS_FOR_UTC_DATETIME_PREF,
                SystemTime.Now().AddDays(-300));
            Assert.Equal(expected, actual);
        ...
        [Fact]
        public void
            MigrateSucessfullyFrom1To2_DateOfTheLastSuccessfulPullFromOld_CURRENT_DAY_TO_DOWNLOAD_KEYS_FOR_UTC_DATETIME_PREF_ToNew_LAST_PULL_KEYS_SUCCEEDED_DATE_TIME()
        {
            //Arrange
            preferences.Clear();
            SystemTime.SetDateTime(testStartDate);
            ...
            preferences.Set(PreferencesKeys.CURRENT_DAY_TO_DOWNLOAD_KEYS_FOR_UTC_DATETIME_PREF,
            ...
            var actual = preferences
                .Get(PreferencesKeys.LAST_PULL_KEYS_SUCCEEDED_DATE_TIME, SystemTime.Now().AddDays(-300))
                .TrimMilliseconds();
            Assert.Equal(expected, actual);
            Assert.False(preferences.ContainsKey(PreferencesKeys.LAST_DOWNLOAD_ZIPS_CALL_UTC_DATETIME_PREF));
            Assert.False(preferences.ContainsKey(PreferencesKeys.CURRENT_DAY_TO_DOWNLOAD_KEYS_FOR_UTC_DATETIME_PREF));
        ...
        [Fact]
        public void
            MigrateSucessfullyFrom0To2_DateOfTheLastSuccessfulPullFromOld_LAST_DOWNLOAD_ZIPS_CALL_UTC_PREF_ToNew_LAST_PULL_KEYS_SUCCEEDED_DATE_TIME()
        {
            //Arrange
            Thread.CurrentThread.CurrentCulture = new CultureInfo("da-DK");
            preferences.Clear();
